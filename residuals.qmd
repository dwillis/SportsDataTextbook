...
We've seen this output before, but let's review because if you are using scatterplots to make a point, you should do this. First, note the Min and Max residual at the top. A team has underperformed the model by 36 points (!), and a team has overperformed it by 35 points (!!). The median residual, where half are above and half are below, is just slightly above the fit line. Close here is good.
...
Look at that Notre Dame-Wisconsin game. Not only did the Badgers outgain the Irish, but the model predicted they would win by a touchdown. Instead, Notre Dame **won by 28**!
...
First, note all of the biggest misses here are all blowout games. The worst games of the season, the worst being Oklahoma vs Western Carolina. The model missed that differential by ... 73 points. The margin of victory? 76 points. In other words, this model is terrible. But let's look at it anyway.
