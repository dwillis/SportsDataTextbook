...
dllink <- download_link(
  link = "https://thescoop.org/sports-data-files/cfbranking22.csv",
...
And the last thing: anyone else annoyed at 7.5th place on the left? We can fix that too by specifying the breaks in scale_y\_reverse. We can do that with the x axis as well, but since we haven't reversed it, we do that in `scale_x_continuous` with the same breaks. Also: forgot my source and credit line.
