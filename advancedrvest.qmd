...
[We can see from this page](https://www.sports-reference.com/cbb/seasons/2022-school-stats.html) that each team is linked. If we follow each link, we get a ton of tables. But they aren't what we need. There's a link to gamelogs underneath the team names.
...
```{r}
#| eval: false
page <- read_html("https://www.sports-reference.com/cbb/schools/abilene-christian/2022-gamelogs.html")
...
First we have to add the root domain to each request. And, each request needs to go to /2022-gamelogs.html instead of /2022.html. If you look at the urls two the page we have and the page we need, that's all that changes.
...
for (i in schools){
  log_url <- gsub("/2022.html","/2022-gamelogs.html", i)
